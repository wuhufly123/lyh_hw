...
#     path = Path(prefix) / f
#     if not path.exists():
#         subprocess.run(['wget', u, '-O', str(path)])
    
#     if path.suffix == ".tgz":
#         subprocess.run(['tar', '-xvf', str(path), '-C', str(prefix)])
#     elif path.suffix == ".zip":
#         subprocess.run(['unzip', '-o', str(path), '-d', str(prefix)])
...
# 首先要训练一个分词器，将文本数据处理一下，进行分词，产生分词模型和分词表tokenizer
vocab_size = 8000
...
task_cfg = TranslationConfig(
        ...
        # data: 数据目录，指定包含预处理后的二进制数据文件的路径。
        # source_lang: 源语言代码，例如 en 表示英语。
        # target_lang: 目标语言代码，例如 zh 表示中文。
        # train_subset: 训练集的子集名称，这里设置为 "train"。
        # required_seq_len_multiple: 序列长度需要是这个值的倍数，通常用于优化计算效率。
        # dataset_impl: 数据集的实现方式，这里设置为 "mmap"，表示使用内存映射（memory-mapped）文件进行数据加载。
        # upsample_primary: 用于数据增强的参数，通常设为 1 表示不进行上采样。就是保持频率不变
...
def train_one_epoch(epoch_itr, model, task, criterion, optimizer, accum_steps=1):
    ...
    for samples in progress:
        ...
        for i, sample in enumerate(samples):
            ...
            # 混合精度训练 autocast将计算转化为float16，提高效率
            with autocast():
...
def inference_step(sample, model):
    ...
    for i in range(len(gen_out)):
        # 对于每个样本，收集输入、假设和参考，稍后用于计算 BLEU，因为生成的都是向量表示，所以需要用decode解码，解码就要给他相应的字典。
        srcs.append(decode(
...
# 该函数用于在每个 epoch 后进行模型验证，并根据需要保存模型检查点和样本
